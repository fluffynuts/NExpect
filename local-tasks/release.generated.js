...
gulp.task("push", "pushes packages to nuget.org", function () {
    ...
    ].flat(), promises = packages.map(pushPackage);
...
gulp.task("test-push", "", function () {
    ...
    for (var k in packages) {
        console.log("".concat(k, ": ").concat(packages[k]));
    }
...
function findPackage(id, ext) {
    return fs
        ...
        .filter(function (p) { return p.endsWith(".".concat(ext)); })
        .filter(function (p) {
        var info = parseNugetPackageFilename(p);
        return info.id == id;
    })
        .map(function (p) { return path.join(packageDir, p); })
        .sort()
        .reverse()[0];
}
var pkgRegex = /^(?<id>[a-zA-Z.]+[a-zA-Z])\.(?<version>[\d.]+[\d]+)-?(?<tag>.*)\.nupkg/;
function parseNugetPackageFilename(filename) {
    var matches = filename.match(pkgRegex);
    return {
