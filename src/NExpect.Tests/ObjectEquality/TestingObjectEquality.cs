...
public class TestingObjectEquality
{
    ...
    [TestFixture]
    public class BoxedEquality
    {
        public static IEnumerable<(object, object)> BoxedTestCases()
        {
            yield return (1, 1L);
            yield return (1L, 1);
            yield return (1, 1UL);
            yield return (1UL, 1);
            yield return (1.1M, 1.1D);
            yield return (1.1F, 1.1D);
            yield return (1.1D, 1.1F);
            yield return (1.1F, 1.1M);
            yield return (1.1M, 1.1F);
        }

        [TestCaseSource(nameof(BoxedTestCases))]
        public void ShouldFindTwoBoxedValuesOfDifferentButCrossAssignableTypesEqualWhenEqual(
            (object, object) testCase
        )
        {
            // Arrange
            var (left, right) = testCase;
        ...
        [Test]
        public void ShouldNotBreakActualEqualityByCasting()
        {
            // Arrange
            var left = 1.1;
            var right = 1L;
            ...
            {
                Expect((object) left)
                    .To.Equal((object) right);
            }, Throws.Exception.InstanceOf<UnmetExpectationException>());
            Assert.That(() =>
            {
                Expect((object) right)
                    .To.Equal((object) left);
            }, Throws.Exception.InstanceOf<UnmetExpectationException>());
            // Assert
        }

        [Test]
        public void ShouldNotAttemptAnyWhackyConversions()
        {
            // Arrange
            var left = true;
            var right = 2;

            Expect(Convert.ChangeType(right, left.GetType()))
                .To.Equal(left);
            // Act
            Assert.That(() =>
            {
